...
namespace Wj.Bizlogic.Uow
{
    public class UnitOfWorkManager : IUnitOfWorkManager, ISingletonDependency
    {
        ...
        public IUnitOfWork Begin(AppUnitOfWorkOptions options, bool requiresNew = false)
        {
        ...
        public void BeginReserved(string reservationName, AppUnitOfWorkOptions options)
        {
            if (!TryBeginReserved(reservationName, options))
            {
                throw new AppException($"Could not find a reserved unit of work with reservation name: {reservationName}");
            }
        }

        public bool TryBeginReserved(string reservationName, AppUnitOfWorkOptions options)
        {
            Check.NotNull(reservationName, nameof(reservationName));
