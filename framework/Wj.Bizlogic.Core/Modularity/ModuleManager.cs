...
namespace Wj.Bizlogic.Modularity
{
    public class ModuleManager : IModuleManager, ISingletonDependency
    {
        ...
        public ModuleManager(
            ...
            IOptions<AppModuleLifecycleOptions> options,
        ...
        public virtual async Task InitializeModulesAsync(ApplicationInitializationContext context)
        {
            foreach (var contributor in _lifecycleContributors)
            {
                foreach (var module in _moduleContainer.Modules)
                {
                    ...
                    {
                        throw new AppInitializationException($"An error occurred during the initialize {contributor.GetType().FullName} phase of the module {module.Type.AssemblyQualifiedName}: {ex.Message}. See the inner exception for details.", ex);
                    }
                }
            }
        ...
        public virtual async Task ShutdownModulesAsync(ApplicationShutdownContext context)
        {
            ...
            foreach (var contributor in _lifecycleContributors)
            {
                foreach (var module in modules)
                {
                    ...
                    {
                        throw new AppShutdownException($"An error occurred during the shutdown {contributor.GetType().FullName} phase of the module {module.Type.AssemblyQualifiedName}: {ex.Message}. See the inner exception for details.", ex);
                    }
                }
            }
        }
