...
namespace Wj.Bizlogic.SimpleStateChecking
{
    ...
    {
        public static string? Serialize<TState>(this ISimpleStateCheckerSerializer serializer, IList<ISimpleStateChecker<TState>> stateCheckers)
        ...
        public static ISimpleStateChecker<TState>[] DeserializeArray<TState>(this ISimpleStateCheckerSerializer serializer, string value, TState state)
            where TState : IHasSimpleStateCheckers<TState>
        {
            ...
            if (array.Count == 1)
            {
                ...
                if (jsonObject == null)
                {
                    throw new AppException("JSON value is not an array of objects: " + value);
                }
